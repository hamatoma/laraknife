...
<form id="file-index" action="/file-index" method="POST">
    ...
    <x-laraknife.panels.index title="{{ __('Files') }}">
      ...
      <x-laraknife.forms.combobox position="first" name="filegroup" label="Filegroup" :options="$optionsFilegroup" class="lkn-autoupdate" width2="4" />
      <x-laraknife.forms.combobox position="last" name="user" label="User" :options="$optionsUser" class="lkn-autoupdate" width2="4" />
      <x-laraknife.forms.string position="first" name="size" label="Size" value="{{ $context->valueOf('size') }}" placeholder="Examples: '<10M' '>=45k' '=5001'" width2="4" />
      <x-laraknife.forms.string position="last" name="created_at" label="Date" value="{{ $context->valueOf('created_at') }}" placeholder="Examples: '<1.1.2024' '>=20.3.2023' '=4.4.2024" width2="4" />
