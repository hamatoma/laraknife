...
@section('content')
<form id="hour-index" action="/hour-index" method="POST">
    @csrf
    <x-laraknife.panels.index title="{{ __('Hours') }}">
      <x-laraknife.panels.filter legend="{{ $pagination->legendText() }}">
      <x-laraknife.forms.combobox position="first" name="hourtype" label="Hourtype" :options="$optionsHourtype" class="lkn-autoupdate" width2="4" />
      <x-laraknife.forms.combobox position="last" name="hourstate" label="Hourstate" :options="$optionsHourstate" class="lkn-autoupdate" width2="4" />
      <x-laraknife.forms.string type="date" position="first" name="from" label="From" value="{{ $context->valueOf('from') }}" width2="4" />
      <x-laraknife.forms.string type="date" position="last" name="until" label="To" value="{{ $context->valueOf('until') }}" width2="4" />
          <x-laraknife.forms.string position="first" name="text" label="Text" value="{{ $context->valueOf('description') }}" width2="4" />
...
@endforeach
        <tr><td></td><td><span class="lkn-text-tagged">{{ __('Sum')}}</span></td><td class="lkn-align-right"><span class="lkn-text-tagged">{{ $context->valueOf('sum') }}</span></td>
        </tr>
      </tbody>
