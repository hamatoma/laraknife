...
<form id="user-editpassword" action="/user-editpassword/{{ $user->id }}" method="POST">
    ...
    <x-laraknife.panels.standard title="{{ __('Change Password of an User') }}" fieldset="false">
        <x-laraknife.layout.nav-tabs :info="$navTabsInfo" fieldset="true">
            ...
            <x-laraknife.forms.password position="first" name="password" label="Password" width2="4" attribute="{!! $context->readonlyUnlessOwner() !!}" />
            <x-laraknife.forms.password position="last" name="password_confirmation" label="Confirmation" width2="4" attribute="{!! $context->readonlyUnlessOwner() !!}" />
