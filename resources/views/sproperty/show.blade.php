...
<form id="sproperty-show" action="/sproperty-show/{{ $sproperty->id }}/{{ $mode }}" method="POST">
    ...
    <x-laraknife.panels.show title="{{ __($mode === 'delete' ? 'Deletion of a Scoped Property' : 'A Scoped Property') }}" mode="{{$mode}}">
        <x-laraknife.forms.text position="first" name="id" label="Id" value="{{ $sproperty->id }}" width2="4"
            attribute="readonly" />
        <x-laraknife.forms.text position="last" name="scope" label="Scope" value="{{ $sproperty->scope }}" width2="4"
            attribute="readonly" />

        <x-laraknife.forms.text position="first" name="name" label="Name" value="{{ $sproperty->name }}"
            width2="4" attribute="readonly"/>
        <x-laraknife.forms.text position="last" name="shortname" label="Shortname" value="{{ $sproperty->shortname }}"
            width2="4" attribute="readonly"/>
        ...
        <x-laraknife.forms.bigtext position="alone" name="info" label="Info" value="{{ $sproperty->info }}"
