...
<form id="sproperty-create" action="/sproperty-store" method="POST">
    ...
    <x-laraknife.panels.create title="{{__('Creation of a Scoped Property')}}">
        ...
        <x-laraknife.forms.string position="last" name="shortname" label="Shortname" value="{{$context->valueOf('shortname')}}" width2="4" />

        <x-laraknife.forms.string position="first" name="order" label="Order" value="{{$context->valueOf('order')}}" width2="4" />
        <x-laraknife.forms.string position="last" name="value" label="Value" value="{{$context->valueOf('value')}}" width2="4" />

        <x-laraknife.forms.text position="alone" name="info" label="Info" value="{{$context->valueOf('info')}}" width2="10" rows="4" />
